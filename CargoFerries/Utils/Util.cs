...
namespace CargoFerries
{
    public static class Util
    {
        ...
        public static bool IsModActive(string modNamePart)
        {
            ...
            {
                UnityEngine.Debug.LogError($"Failed to detect if mod with name containing {modNamePart} is active");
        ...
        public static bool IsModActive(ulong modId)
        {
            try
            {
                var plugins = PluginManager.instance.GetPluginsInfo();
                return (from plugin in plugins.Where(p => p.isEnabled)
                    select plugin.publishedFileID into workshopId
                    where workshopId.AsUInt64 == modId select workshopId).Any();
            }
            catch (Exception e)
            {
